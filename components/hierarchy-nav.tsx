...
// District interface removed - no longer needed
...
interface Hospital {
  ...
  district_id: number  // 保留district_id用于数据库关联，但在UI中不再直接使用
}
...
export const HierarchyNav = forwardRef<HierarchyNavRef, { onSelectHospital: (hospitalId: number) => void }>(
  ...
  const fetchHospitalsByCity = async (city: City) => {
    try {
      setLoading(true)
      setError(null)
      ...
      const response = await fetch(
        `${API_BASE_URL}/hospitals?city=${encodeURIComponent(city.name)}&page=1&page_size=100`,
      )
      const data = await response.json()
  ...
  const refreshHospitalList = async () => {
    // 如果当前选中的是某个城市，重新获取医院列表
    if (selectedCity) {
      await fetchHospitalsByCity(selectedCity)
    }
  }
  ...
  const returnToHospitalList = () => {
    ...
    // 如果之前已经选择了城市，直接返回到医院的tab
    if (selectedCity) {
      console.log('✅ Returning to hospitals tab for city:', selectedCity.name);
      setActiveTab("hospitals");
    }
    // 如果只选择了省份，返回到城市tab
  ...
  const breadcrumbPath = [
    ...
    ...(selectedProvince
      ...
      : []),
    ...(selectedCity ? [{ label: selectedCity.name }] : []),
  ]
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-card">
          ...
          <TabsTrigger value="hospitals" disabled={!selectedCity}>
            医院
          </TabsTrigger>
        </TabsList>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {cities.map((city) => (
                <Card
                  ...
                  onClick={() => fetchHospitalsByCity(city)}
                >
                  <div className="flex items-center justify-between">
                    <div>
          ...
          {selectedCity && (
            <div>
              <div className="px-4 py-2 bg-primary/10 text-primary rounded text-sm font-medium mb-4">
                {selectedCity.name} 的医院列表 ({filteredHospitals.length})
              </div>
